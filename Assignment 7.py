...
import asyncio
from binance.websockets import BinanceSocketManager #SOLUTION: https://stackoverflow.com/questions/60896486/how-to-fix-errors-when-running-pip-install-python-binance
...
import plotly #Ref: https://plotly.com/python/getting-started/
import chart_studio
...
def exercise01():
    ...
    df_first_five_rows = df.head() #head() by default returns the 1st five rows
...
def exercise02(new_observations):
    ...
    # ------ Place code below here \/ \/ \/ ------
    knn = KNN() #neighbors=5 by default
    knn.fit(iris['data'],iris['target'])
    pred = knn.predict(new_observations)
    iris_predictions = iris['target_names'][pred]

    ##For KNN / fitting:
    #Refer to Datacamp course 'Supervised learning with sci kit learn' notes
    
    ##For making predictions:
    #Ref: https://www.kaggle.com/ghollah/iris-prediction (scroll to 'Making Predictions' portion)
...
def exercise03(neighbors,split):
    ...
    '''
    #random_state = 21 #noted :)

    # ------ Place code below here \/ \/ \/ ------

    X_train, X_test, y_train, y_test = tts(iris['data'], iris['target'], test_size=split, random_state = 21, stratify=iris['target'])
    knn = KNN(neighbors)
...
def exercise04():
    ...
    # ------ Place code below here \/ \/ \/ ------
    X_train, X_test, y_train, y_test = tts(iris['data'], iris['target'], test_size=.25, random_state = 21, stratify=iris['target'])
    
    neighbors = np.arange(1, 30) #list of integers from 1 to 30
    ...
    fig.add_trace(go.Scatter(x=neighbors, y=train_accuracy, name="train"))
    fig.add_trace(go.Scatter(x=neighbors, y=test_accuracy, name="test"))
    fig.update_layout(title="knn score vs. # of neighbors", xaxis_title = "neighbors (k)", yaxis_title = "knn score")

    plotly_overfit_underfit_curve_url = py.plot(fig, filename= 'knn-line')
    # ------ Place code above here /\ /\ /\ ------
...
def exercise05():
    ...
    BOS_tv = pd.DataFrame(boston.target, columns=["MEDV"])

    average_age = np.mean(BOS_data['AGE'])
    average_medv = np.mean(BOS_tv['MEDV'])
    medv_as_numpy_values = BOS_tv.values #removed .values() and 'MEDV'
...
def exercise06():
    ...
    X_ptratio = np.asarray(BOS_data['PTRATIO'].values)
    medv_np = np.asarray(BOS_tv.values)

    #Plot it - COMMENTED OUT matplotlib plots for sake of compilation, time, etc.
    # plt.scatter(X_ptratio, medv_np, color='blue')
    # plt.title('Boston dataset: median home value vs. pupil-teacher ratio scatterplot')
...
def exercise07():
    ...
    X_PT = BOS_data['PTRATIO'].values[:, np.newaxis] #X: convert 1D to 2D array
    y_MEDV = BOS_tv.values #y

    reg = lm.LinearRegression().fit(X_PT, y_MEDV)
    prediction_space = np.linspace(min(X_PT),max(X_PT)) #got rid of .reshape(-1,1)
    reg_model = reg.predict(prediction_space)

    #PLOT: this isn't perfect but it's a good 1st attempt. There's likely a simpler method for adding a fit line
    ##ie. https://stackoverflow.com/questions/19068862/how-to-overplot-a-line-on-a-scatter-plot-in-python
    # x = np.arange(12,22,10/len(reg_model))
    # X_ptratio = np.asarray(BOS_data['PTRATIO'].values)
    # medv_np = np.asarray(BOS_tv.values)

    # plt.scatter(X_ptratio, medv_np, color='blue')
    # plt.plot(x, np.array(reg_model))
    # plt.title('Boston dataset: median home value vs. pupil-teacher ratio scatterplot')
    # plt.xlabel('Pupil-teacher ratio (PTRATIO)')
    # plt.ylabel('Median home value (MEDV)')
    # plt.show()
...
def exercise08():
    ...
    #Because this was not relevant for the final and I time became of the essence, I bypassed this exercise ...
