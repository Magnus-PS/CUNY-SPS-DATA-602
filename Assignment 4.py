...
class PandasChain:
    ...
    def display_chain(self): 
        for x in range(self.get_number_of_blocks()):
            self.__chain[x].display_transactions() #for each block in the chain, display all transactions
    ...
    def __commit_block(self, block): 
        ...
        # Obtain merkle root hash, generate and set block's hash
        merkle_root = block.get_simple_merkle_root()
        
        # Calculate block hash
        block_hash = block.set_block_hash(merkle_root)
        
        # Set prev_hash to current block's hash
        ...
        # Create new block as current block
        self.__current_block = Block(self.__seq_id, self.__prev_hash) # Create new Block 
    ...
    def display_block_headers(self): 
        for x in range(self.get_number_of_blocks()):
            self.__chain[x].display_header() #for each block in the chain, display header
    
    # 5 pts - return int total number of blocks in this chain (committed and uncommitted blocks combined)
    def get_number_of_blocks(self):
        
        #intialize with +1 since we don't add to the chain until we've commited
        #thus we're considering uncommited blocks as well by taking +1
        block_number = len(self.__chain) + 1
        return int(block_number)
    
    # 10 pts - Returns all of the values (Pandas coins transferred) of all transactions from every block as a single list
    def get_values(self):
        
        value_list = [] #initialize empty list to hold values
        
        #Iterate through each block within the chain, get values and append each value to the list
        for block in self.__chain:
            block_vals = block.get_values()
            [value_list.append(x) for x in block_vals]        
...
class Block:
    ...
    def display_header(self): 
        print("Sequence ID:", self.__seq_id, "Status:", self.__status, "Block hash:", self.__block_hash, "Previous block's hash:", self.__prev_hash, "Merkle hash:", self.__merkle_tx_hash, "Number of transations:", self.get_size())
    
    # 10 pts - This is the interface for how transactions are added
    def add_transaction(self,s,r,v): 
        ct = dt.datetime.now() #First get current time
        ts = ct.timestamp() # Then convert to timestamp 
        
        # Hash timestamp, sender, receiver, value
    ...
    def display_transactions(self): 
        for x in range(self.__transactions.get_size()):
            print(str(self.__transactions['TxHash'[x]])) #display each transaction in the block
    
    # 5 pts- Return the number of transactions contained by this block
    def get_size(self): 
        size = len(self.__transactions)
        return int(size) #Return the length of the block as an integer
    ...
    def set_status(self, status):
        if(status == 'COMMITTED'):
            self.__status = 'COMMITTED' # Change from initial status
        #Otherwise, do nothing since the initial status was UNCOMMITTED
    
    # 5 pts - Setter for block hash
    def set_block_hash(self, hash):
        blk_hash = hashlib.sha256(str(str(hash)+str(self.__prev_hash)).encode('utf-8')).hexdigest()
    ...
    def get_simple_merkle_root(self): 
        #Create a list of the transaction hashes for the block
        my_list = list(self._Block__transactions['TxHash'])
        
        #Concatenate list of transactions as string with '' as separator
        #Ref https://www.decalage.info/en/python/print_list
        full_string = ''.join(map(str, my_list))

        self.__merkle_tx_hash = hashlib.sha256(full_string.encode('utf-8')).hexdigest() #hash to produce simple merkle root. do I need to encode to UTF?
        return self.__merkle_tx_hash #return the value
    
    def get_values(self):
        #Create a list of values for the block
        list_bvals = list(self._Block__transactions['Value'])
        return list_bvals
...
class TestAssignment4(unittest.TestCase):
    def test_chain(self):
        ...
        x = list(range(1,21))
        y = pandas_chain.get_values() #test get_values() function of pandas_chain
        plt.plot(x, y)
        plt.axis([0, 21, 45, 55])
        plt.xlabel("index")
        plt.ylabel("value")
