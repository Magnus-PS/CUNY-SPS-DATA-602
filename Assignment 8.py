...
class Iris:
    ...
    def __init__(self,creds,dbname='data602',new=True):
        self.__conn = self.__get_connection(creds) # connect and store the MySQL connection object 
        self.__dbname = dbname # store the database name

        #Establish connection using credentials 
        ##cursors allow Python code to execute SQL
        mycursor = self.__conn.cursor()

        if new:
            # if new, create database and table
            self.__create()
        else:
    ...
    def __create(self):
        ...
        #Drop DB and table - to deal with exceptions / raised errors: try except
        #ref: https://www.w3schools.com/python/python_try_except.asp#:~:text=The%20try%20block%20lets%20you,the%20try%2D%20and%20except%20blocks.
        ...
        mycursor.execute("CREATE TABLE iris_data (id INT NOT NULL, feature_sepal_length FLOAT NOT NULL, feature_sepal_width FLOAT NOT NULL, feature_petal_length FLOAT NOT NULL, feature_petal_width FLOAT NOT NULL, target_species VARCHAR(20) NOT NULL, target_species_id INT NOT NULL)")
        self.__conn.commit() #commit changes to DB
    ...
    def close(self):
        # ------ Place code below here \/ \/ \/ ------
        self.__conn.close() #In our given test output I didn't see 'Disconnected' so I didn't use ... likely an oversight / error on my part
        # ------ Place code above here /\ /\ /\ ------
        print('Disconnected')
    ...
    def load(self,truncate=False):
        ...
        df["Species"] = pd.Categorical.from_codes(iris.target, iris.target_names) #derive species (setosa, virginica, etc.) using species id: https://pasteboard.co/HPCJOiI.png
        df["SpeciesId"] = iris.target
        #print(df.head()) #verify the conversion

        ##For loading data from sklearn dataset and converting to pandas (scroll down):
        ...
        #Next we utilize panda's iterrows() function to iterate over DF rows as (index, Series) pairs.
        ##a Series is a 1D labeled array that can hold data of any type and be accessed via axis labels (ie. "sepal length (cm)")
    ...
    def display_gt(self,n): 
        ...
        #Establish connection using credentials 
        mycursor = self.__conn.cursor(buffered=True) #it seems once the data's been entered, we're to use buffered=True to access it in an iterative manner ...
        mycursor.execute("SELECT * FROM iris_data WHERE id > {}".format(n))
        display = mycursor.fetchall() #assign all rows from SQL B to display
    ...
    # Delete all rows that are in the row_ids list  
    ...
    def get_row_count(self):
        ...
        count = mycursor.fetchone()[0] #extract count # from "()" around it, otherwise we get an error 
