...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    temperatures.extend(iot_sensor_points.values()) #pull values from dictionary
    
    single_temps = [a,b,c,d,e]
    temperatures.extend(single_temps)
    
    temperatures.sort(reverse=True)
    
    samples = temperatures[4::5] #return a list with every nth elements, starting at 4
...
def exercise10(sentence):
    ...
    # ------ Place code below here \/ \/ \/ ------
    scored = sentence.replace(' ', '_')
    swapped = scored.swapcase()
    reversed = swapped[::-1]
    # ------ Place code above here /\ /\ /\ ------
    return reversed
